        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item"><a href="skillmap.php">C&C Access</a></li>
              <li class="breadcrumb-item active">Skill Up</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
...
<?php
  ...
  <a href="../skillup/skillup.php?NIK=<?php echo $d['NIK']; ?>" class="btn btn-info btn-block margin-bottom">Skill Up</a> 
  <a href="<?php if ($d['sub_section'] == 'Auto') { echo '../skillmap/C&Cskill.php?NIK=' . $d['NIK']; } else { echo '../skillmap/postSkill.php?NIK=' . $d['NIK']; } ?>" class="btn btn-primary btn-block margin-bottom">Skill Map</a>
  </div>

  <div class="col-md-9">
          <div class="card card-info">
            <div class="card-header">
              <h3 class="card-title">Skill Up</h3>
              ...
              <div class="card-body">
              <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target=".modalBesar<?php echo $d['NIK']; ?>"><i class="fas fa-plus">Tambah Data</i></button>
   ...
   <div class="modal-dialog modal-lg">
     ...
     <div class="modal-content">
       <div class="modal-header">
         <h5>Skill Up a/n <?php echo $d['nama']; ?>,<?php echo $d['NIK']; ?> </h5>
       </div>
       <div class="modal-body">
       <div class="card card-success">
       <div class="card-header">
          <h3 class="card-title"><center>Skill Up</center></h3>
        </div>
       <div class="card-body">
       <div class="row">
                  <div class="col-md-6">
                  <form method="post" action="tambah_skillup.php">
                    <div class="form-group">
                      <label for="exampleInputEmail1">NIK</label>
                      <input type="number" class="form-control" id="NIK" name="NIK" placeholder="NIK" readonly value="<?php echo $d['NIK']; ?>">
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail1">Training Name</label>
                      <input type="text" class="form-control" id="NIK" name="trainingmesin" placeholder="Training Name">
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail1">Date</label>
                      <input type="date" class="form-control" id="NIK" name="date" placeholder="date of training">
                    </div>
                    </div>
                    <div class="col-md-6">
                    <div class="form-group">
                      <label for="exampleInputEmail1">Score and result for Evaluasi Teori</label>
                      <input type="text" class="form-control" id="NIK" name="teori" placeholder="Score of teori">
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail1">Score and result for Evaluasi Practice</label>
                      <input type="text" class="form-control" id="NIK" name="practice" placeholder="Score of practice">
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail1">Trainer</label>
                      <input type="text" class="form-control" id="NIK" name="trainer" placeholder="Trainer Name">
                    </div>
                    </div>
	...
	?>
         <center> <h4>PT.Sumitomo Wiring Systems Batam Indonesia</h4>
        <h5>Training Centre Skill Up </h5> </center>
        <table id="" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <th colspan="3"><center>Score and Result for Evaluasi Training</center></th>
                    <th rowspan="5"></th>
                  </th>
    <tr>
      <th>Score Of Teori</th>
      <th>Score Of Practice</th>
      <th>Result</th>
    ...
    <?php 
				...
				$data = mysqli_query($db,"select * from skillupcc where NIK='$NIK'");
				$jumlah_data = mysqli_num_rows($data);
				$total_halaman = ceil($jumlah_data / $batas);
 
				$data_pegawai = mysqli_query($db,"select * from skillupcc where NIK='$NIK' limit $halaman_awal, $batas");
    ...
    <tbody>
                  <tr>
                    ...
                    <td><?php echo $d['trainingmesin']; ?></td>
                    <td><?php echo $d['date']; ?></td>
                    <td><?php echo $d['trainer']; ?></td>
                    <td><?php echo $d['teori']; ?></td>
                    <td><?php echo $d['practice']; ?></td>
                    <?php
                      $nilai_practice = (int)$d['practice'];
                      $nilai_teori = (int)$d['teori'];

                      if (($nilai_practice > 80) && ($nilai_teori == 100)) {
                      echo '<td style="background-color: palegreen;">Lulus</td>';
                      } else {
                      echo '<td>Tidak Lulus</td>';
                      }
                    ?>
                     <td><button type="button" class="btn btn-outline-dark btn-sm" data-toggle="modal" data-target="#modal-danger<?php echo $d['id']; ?>"><i class="fas fa-trash"></i>
                </button>
